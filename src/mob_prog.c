...
char *mprog_next_command(char*);
bool mprog_seval(char*, char*, char*);
bool mprog_veval(int, char*, int);
bool mprog_do_ifchck(char*, CHAR_DATA*, CHAR_DATA*, OBJ_DATA*, void*, CHAR_DATA*);
char *mprog_process_if(char*, char*, CHAR_DATA*, CHAR_DATA*, OBJ_DATA*, void*, CHAR_DATA*);
void mprog_translate(char, char*, CHAR_DATA*, CHAR_DATA*, OBJ_DATA*, void*, CHAR_DATA*);
void mprog_process_cmnd(char*, CHAR_DATA*, CHAR_DATA*, OBJ_DATA*, void*, CHAR_DATA*);
void mprog_driver(char*, CHAR_DATA*, CHAR_DATA*, OBJ_DATA*, void*);
...
void mprog_greet_trigger(CHAR_DATA *mob)
{
    ...
    {
        if (IS_NPC(vmob) && mob != vmob && can_see(vmob, mob)
            && (vmob->fighting == NULL) && IS_AWAKE(vmob) && (vmob->pIndexData->progtypes & GREET_PROG))
        {
            mprog_percent_check(vmob, mob, NULL, NULL, GREET_PROG);
        }
        else if (IS_NPC(vmob) && (vmob->fighting == NULL) 
            && IS_AWAKE(vmob) && (vmob->pIndexData->progtypes & ALL_GREET_PROG))
        {
