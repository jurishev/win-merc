...
void do_goto(CHAR_DATA *ch, char *argument)
{
    ...
    {
        act("$n $T.", ch, NULL, (ch->pcdata != NULL && ch->pcdata->bamfout[0] != '\0')
            ? ch->pcdata->bamfout 
            : "leaves in a swirling mist", TO_ROOM);
    }
    ...
    if (!IS_SET(ch->act, PLR_WIZINVIS))
    {
        act("$n $T.", ch, NULL, (ch->pcdata != NULL && ch->pcdata->bamfin[0] != '\0')
            ? ch->pcdata->bamfin 
            : "appears in a swirling mist", TO_ROOM);
    }
...
void do_mstat(CHAR_DATA *ch, char *argument)
{
    ...
    sprintf(buf, "Lv: %d.  Class: %d.  Align: %d.  AC: %d.  Gold: %d.  Exp: %d.\r\n",
