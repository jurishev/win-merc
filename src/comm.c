...
void game_loop_unix( SOCKET );
SOCKET init_socket( void );
void new_descriptor( SOCKET );
bool read_from_descriptor( DESCRIPTOR_DATA* );
bool write_to_descriptor( SOCKET, char*, int );
...
int main( void ) {
	...
	fprintf( stderr, "Initializing server socket\n" );

	SOCKET control = init_socket();

	fprintf( stderr, "Booting database\n" );

	boot_db();

	sprintf( log_buf, "Merc is ready to rock on port %s.", port );
	log_string( log_buf );
...
SOCKET init_socket() {
	...
	// Регистрация локального адреса и порта
	...
	// Связывание сокета с локальным адресом из result
    ...
    // Содержимое result больше не нужно, освобождаем память
    
    freeaddrinfo( result );
    
    // Прослушивание очереди подключений
...
void game_loop_unix( SOCKET control ) {

	// Внимание на функцию gettimeofday()
	// В стандартных библиотеках Си такой нет
	// #include <sys/time.h>
	// Она используется для получения значений в структуру TIMEVAL
	// При этом current_time можно получить стандартно через time( &current_time )
	...
	while( !merc_down ) {
		...
		if ( select( 0, &in_set, &out_set, &exc_set, &null_time ) == SOCKET_ERROR ) {
...

		{
			...
			if ( secDelta > 0 || ( secDelta == 0 && usecDelta > 0 ) ) {
				...
				/* Не знаю, как под юниксами, а винда требует, чтобы select()
				принимала хотя бы один набор, и чтобы в нем был хотя бы один сокет. */
				
				FD_ZERO( &exc_set );
				FD_SET( control, &exc_set );

				if ( select( 0, NULL, NULL, &exc_set, &stall_time ) == SOCKET_ERROR ) {
...
void new_descriptor( SOCKET control ) {
	...
	getsockname( control, ( SOCKADDR* ) &sock, &size );

	SOCKET desc = accept( control, ( SOCKADDR* ) &sock, &size );
...
bool read_from_descriptor( DESCRIPTOR_DATA *d ) {
	...
	do {
		...
		int nRead = recv( d->descriptor, d->inbuf + iStart, sizeof( d->inbuf ) - 10 - iStart, 0 );

		if ( nRead > 0 )
			iStart += nRead;
		else if ( nRead == 0 ) {
			log_string( "Client disconnected" );
			return false;
...
bool process_output( DESCRIPTOR_DATA *d, bool fPrompt ) {
	...
	if ( fPrompt && !merc_down && d->connected == CON_PLAYING )
		...
		if ( d->showstr_point )
			write_to_buffer( d, 
				"[Please type (c)ontinue, (r)efresh, (b)ack, (h)elp, (q)uit, or RETURN]:  ", 0 );
		else {
...
bool write_to_descriptor( SOCKET desc, char *txt, int length ) {
	...
	switch ( d->connected ) {
		...
		case CON_GET_NAME:
			...
			if ( check_reconnect( d, argument, false ) )
				fOld = true;
			else if ( wizlock && !IS_HERO( ch ) && !ch->wizbit ) {
		...
		case CON_READ_MOTD:
			...
			for ( NOTE_DATA *pnote = note_list; pnote != NULL; pnote = pnote->next )
